...
/**
 * @brief Get a gpio_ranges specifier's cell value at an index
 *
 * Example devicetree fragment:
 *
 ...
 *
 *     n: node {
 *             gpio-ranges = < &pinctrl 0 16 14 >, < &pinctrl 2 32 16 >;
 *     };
 *
 * Bindings fragment for the vnd,pinctrl compatible:
 *
 *     gpio-range-cells:
 *       - gpio_offset
 *       - pin_offset
 ...
 * Example usage:
 *
 *     DT_GPIO_RANGES_CELL_BY_IDX(DT_NODELABEL(n), 0, gpio_offset) // 0
 *     DT_GPIO_RANGES_CELL_BY_IDX(DT_NODELABEL(n), 1, pin_offset) // 32
 *     DT_GPIO_RANGES_CELL_BY_IDX(DT_NODELABEL(n), 0, npins) // 14
 *
 * @param node_id node identifier for a node with a gpio-ranges property
 * @param idx logical index into gpio-ranges property
 * @param cell lowercase-and-underscores cell name
 * @return the cell value at index "idx"
 * @see DT_PHA_BY_IDX()
 */
#define DT_GPIO_RANGES_CELL_BY_IDX(node_id, idx, cell) \
	DT_PHA_BY_IDX(node_id, gpio_ranges, idx, cell)

/**
 * @brief Get a DT_DRV_COMPAT instance's gpio_ranges specifier's cell value
 *        at an index
 * @param inst DT_DRV_COMPAT instance number
 * @param idx logical index into gpio_ranges property
 * @param cell lowercase-and-underscores cell name
 * @return the cell value at index "idx"
 * @see DT_GPIO_RANGES_CELL_BY_IDX()
 */
#define DT_INST_GPIO_RANGES_CELL_BY_IDX(inst, idx, cell) \
	DT_GPIO_RANGES_CELL_BY_IDX(DT_DRV_INST(inst), idx, cell)

/**
 * @brief Equivalent to DT_INST_GPIO_RANGES_CELL_BY_IDX(inst, 0, cell)
 * @param inst DT_DRV_COMPAT instance number
 * @param cell lowercase-and-underscores cell name
 * @return the value of the cell inside the specifier at index 0
 */
#define DT_INST_GPIO_RANGES_CELL(inst, cell) \
	DT_INST_GPIO_RANGES_CELL_BY_IDX(inst, 0, cell)
/**
 * @}
 */
